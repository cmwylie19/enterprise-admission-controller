...
Your boss is pleased at the enhanced security posture, but you noticed the Pod and Container securityContexts are all over the map. There is no standardization across teams. Some pods are running as user 0 (which is a problem), some are running as user 655532, and some are running as 1000.
...
Create a new action to Mutate pods to have a default runAsUser value:
...
To check if your module correctly rejects privileged pods:
1. Open a javascript debug terminal and run `npx pepr dev --confirm`
...
mutate-to-default pod should have mutated both the pod and the container to 655532 since it was set
...
If you got the expected output, you win! Now you have mutated the pods creating a standard for runAsUser. Go to the next. **DO NOT DELETE YOUR CLUSTER**
